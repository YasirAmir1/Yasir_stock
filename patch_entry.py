...
new_cust = """      {/* Customer Info Box */}
      <div className="bg-slate-50 border border-slate-300 rounded-xl p-3 sm:p-4 shadow-sm">
        <h3 className="text-sm font-black text-slate-800 mb-3 flex items-center gap-2">
          معلومات الزبون
        </h3>
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          <div>
            <label className="block text-[10px] sm:text-xs font-bold text-slate-700 mb-1">اسم الزبون (إلزامي)</label>
            <input
              type="text"
              value={customerName}
              onChange={(e) => setCustomerName(e.target.value)}
              placeholder="اسم الزبون..."
              className="w-full px-2 py-1.5 sm:px-3 sm:py-2 border border-slate-300 rounded-lg text-[10px] sm:text-xs font-bold text-slate-900 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
          </div>
          <div>
            <label className="block text-[10px] sm:text-xs font-bold text-slate-700 mb-1">كود الزبون</label>
            <input
              type="text"
              value={customerCode}
              onChange={(e) => setCustomerCode(e.target.value)}
          ...
          <div>
            <label className="block text-[10px] sm:text-xs font-bold text-slate-700 mb-1">العنوان</label>
            <input
              type="text"
              value={customerAddress}
              onChange={(e) => setCustomerAddress(e.target.value)}
